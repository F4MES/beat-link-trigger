...
- The beat-carabiner library now allows the Ableton Link timeline to be nudged by tempo to keep it close to the CDJs for small adjustments, which can provide a much cleaner-sounding tracking. This still needs to be documented.
...
- We try to handle unexpected (new) track banks and phrase types in rekordbox phrase analysis gracefully, by allowing them to match phrase triggers that are configured to accept all known track banks or phrase types.
...
- When we are processing precise position packets, though, we are now able to track the playback of even unanalyzed tracks on the CDJ-3000 pretty well.
- The user guide now includes link to Ineffable's great video tutorial for synchronizing videos in Resolume.
...
> [!IMPORTANT]
> If you previously gave up on using BLT in real-player-number mode (for example to allow an Ableton Link session to control CDJ tempo) because it caused issues with your CDJs, it is worth testing again, the first fix should make it work smoothly. If not, please [let us know](https://deep-symmetry.zulipchat.com/#narrow/stream/275322-beat-link-trigger).
...
- The interface for loading tracks on CDJ-3000s should work again even when you are using a real player number.
...
- When unanalyzed tracks were loaded on a player, Beat Link was inappropriately looking for matches using Crate Digger in the rekordbox database, which would display incorrect title, artist, and album art for the tracks.
...
- Trying to simulate a show cue expression from within the expression editor would not have access to the show.
