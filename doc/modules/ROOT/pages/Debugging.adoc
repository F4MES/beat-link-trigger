...
But be careful if you are inside a show's namespace when you close the show! That deletes the namespace, which means none of the friendly functions you were relying on, including `(in-core-ns)` exist for your REPL any more!
Reopening the show won't help either, because that creates a new unique namespace for the new instance.
You will need to use an Emacs function like `cider-repl-set-ns` (normally bound to kbd:C-c M-n[]) to switch back to `beat-link-trigger.core`.
If you're using Cursive, you may need to close your remote REPL connection and start a new one.
====
